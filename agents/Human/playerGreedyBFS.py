...
class myAgent(Agent):
    ...
    def SelectAction(self, actions, game_state):
        ...
        player4State = deepcopy(state)
        player3State = deepcopy(state)
# Player 3 ------------------------------------------------------------
        highest_score = 0
        best_move = actions[0]
        player3BestState = []
        #TODO FIGURE OUT BETTER WAY TO DEAL WITH JACK
        for action in actions[0:400]:
            #board = state.board.chips
            #card = action['play_card']
            draft = action['draft_card']
            move = action['type']

            if move == 'trade:' and draft:
                # player3State = deepcopy(state)
                player3State.agents[agent_id].hand = [draft]
                hypothetical_actions = self.getLegalActions(player3State, agent_id)
                for hypo_action in hypothetical_actions:
                    succ_state = self.generateSuccessor(player3State,hypo_action,agent_id)
                    ...
                    if succ_score > highest_score:
                        ...
                        player3BestState = succ_state
            else:
                player3State = deepcopy(state)
                succ_state = self.generateSuccessor(player3State,action,agent_id)
                succ_score = get_win_percentage(succ_state.board.chips, your_char, their_char, special_char)
                if succ_score > highest_score:
                    best_move = action
                    highest_score = succ_score
                    player3BestState = succ_state
        player3Action=best_move
#  Agent 4--------------------------------------------------       
        ...
        player4State.board.draft = ['2s']
        nodraft_actions = self.getLegalActions(player4State, agent_id)
        best_card = None
        best_cord = None
        best_state = None
        
        for action in nodraft_actions:
            play_hypothetical = deepcopy(player4State)
...
# ----------------------------------------------------------------------------
        # BFS EXPAND FUNCTION
        def expand_node(BFSstate,depth):
            # this sets depth of search (cards in hand left)
            if len(BFSstate.agents[agent_id].hand) == depth:
                return []
        ...
        while not myqueue.empty():
            ...
            if BFSstate not in visited:
                ...
                succNodes = expand_node(BFSstate,4)
                if not succNodes:
        ...
        for action in best_state_actions:
            ...
            if succ_score > future_highest_score:
                best_draft_card = action['play_card']
                future_highest_score = succ_score
                bfs4BestState = succ_state

        bfs4Action = {'play_card':best_card, 'draft_card':best_draft_card, 'type':best_type, 'coords':best_cord}
        # --------------------------------------------------------------
        # BFS depth5 ---------------------------------------
        ...
        while not myqueue.empty():
            ...
            if BFSstate not in visited:
                visited.add(BFSstate)
                succNodes = expand_node(BFSstate,3)
                if not succNodes:
                    BFSpath = BFSpath + [(BFSstate, BFSaction)]
                    #print(get_win_percentage(BFSstate.board.chips, your_char, their_char, special_char))
                    final_nodes.append((BFSstate, BFSpath))
        ...
        for action in best_state_actions:
            ...
            if succ_score > future_highest_score:
                best_draft_card = action['play_card']
                future_highest_score = succ_score
                bfs5BestState = succ_state

        bfs5Action = {'play_card':best_card, 'draft_card':best_draft_card, 'type':best_type, 'coords':best_cord}
        # --------------------------------------------------------------
        print("player3:", player3Action)
        print("player4:", player4Action)
        print("player6:", player6Action)
        print("BFS4:",bfs4Action,bsf4_path)
        print("BFS5:",bfs5Action,bsf5_path)

        p3hand = player3BestState.agents[agent_id].hand
        p4hand = player4BestState.agents[agent_id].hand
        p6hand = player6BestState.agents[agent_id].hand
        bfs4hand = bfs4BestState.agents[agent_id].hand
        bfs5hand = bfs5BestState.agents[agent_id].hand

        data = [p3hand,p4hand,p6hand,bfs4hand,bfs5hand,player3Action,player4Action,player6Action,bsf4_path,bsf5_path]
