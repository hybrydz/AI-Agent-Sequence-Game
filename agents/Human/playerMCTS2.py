...
class StateNode(Node):
    ...
    def __init__(self, mdp, parent, state, agent_id, reward = 0, probability = 1.0, bandit = UpperConfidenceBounds(), ):
    ...
    def select(self):
        ...
        if not self.isFullyExpanded():
            ...
            qValues = dict()
            for action in actions:
                #TODO HOW IS THE VALUE OF THIS NODE updated
    ...
    def expand(self):
        ...
        actions = my_get_legal_actions(self.state, self.agent_id) - self.children.keys()
        action = random.choice(list(actions))

        #choose an outcome
        newChild = EnvironmentNode(self, self.state, action)
...
class EnvironmentNode(Node):
    ...
    def select(self):
        ...
        newState = my_get_successor_state(self.state, self.action, self.agent_id)
    ...
    def addChild(self, action, newState, reward):
        child = StateNode(self, newState, reward)
        self.children += [child]
        return child

    def expand(self):
        # choose one outcome based on transition probabilities
        (newState, reward) = my_get_successor_state(self.state, self.action, self.agent_id)

        # expand all outcomes
        return self.addChild(self.action, newState, reward)
...
def opponentMoveGenerator(state, agent_id, moves):
    ...
    for action in getLegalActions(opp_state, opp_id):
        ...
        succ_state = generateSuccessor(hypostate, action, opp_id)
...
def MDPexecute(state, action, agent_id, moves):
...
def my_get_successor_state(state, action, agent_id):
    ...
    return new_state, get_win_percentage(new_state.board.chips, your_char, their_char, special_char)
    ...
    def SelectAction(self, actions, game_state):
        ...
        if actions[0]['type'] == 'trade':
            for action in actions:
                ...
                hypothetical_actions = getLegalActions(hypothetical, agent_id)
                for hypo_action in hypothetical_actions:
                    succ_state = generateSuccessor(hypothetical,hypo_action,agent_id)
