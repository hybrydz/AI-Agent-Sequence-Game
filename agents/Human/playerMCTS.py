...
class Node:
    ...
    def expand(self, state, action_probs_dict):
        """
        We expand a node and keep track of the prior policy probability given by neural network
        """
        self.state = deepcopy(state)
        for action in action_probs_dict:
            self.children[action] = Node(prior=action_probs_dict[action][0])
            self.children[action].state = action_probs_dict[action][1]
...
class MCTS:
    ...
    def moves_value_dict(self, myagent, state, agent_id, your_char, their_char, special_char):
        # output array of legal actions and their probability
        actions = myAgent.getLegalActions(myagent, state, agent_id)
        ...
        for action in actions:
            #TODO should I store this succ state in children?
            succ_state = myAgent.generateSuccessor(myagent, state, action, agent_id)
            succ_state.agents[agent_id].hand.remove('##')
            succ_state.board.draft = ['##']
            score = get_win_percentage(succ_state.board.chips, your_char, their_char, special_char)
            score_actions_dict[action] = (score, deepcopy(succ_state))
            total_score += score

        action_probs_dict = {action: ((score / total_score), succ_state) for action, (score, succ_state) in score_actions_dict.items()}

        return action_probs_dict

    def run(self, myagent, state, agent_id, your_char, their_char, special_char):

        root = Node(0)

        # EXPAND root
        action_probs_dict = self.moves_value_dict(self, myagent, state, agent_id, your_char, their_char, special_char)

        #TODO check if valid probability distribution without this statement below
        #action_probs /= np.sum(action_probs)
        root.expand(state, action_probs_dict)
        ...
        for i in range(self.args['num_simulations']):
            ...
            if not node.state:
                next_state = myAgent.generateSuccessor(myagent, state, action, agent_id)
            else:
                next_state = deepcopy(node.state)

            if len(next_state.agents[agent_id].hand < 3):
                value = get_win_percentage(next_state.board.chips, your_char, their_char, special_char)
            else:
                #TODO PUT IN WIN CONDITION CHECK
                value = None

            if value is None:
                action_probs_dict = self.moves_value_dict(self, myagent, next_state, agent_id, your_char, their_char, special_char)
                node.expand(next_state, action_probs_dict)
...
class myAgent(Agent):
    ...
    def SelectAction(self, actions, game_state):
        ...
        def expand_node(BFSstate):
            # this sets depth of search (cards in hand left)
            if len(BFSstate.agents[agent_id].hand) == 3:
                return []
        ...
        # TODO FIGURE OUT TO IMPLEMENT DRAFT
        #hypothetical.agents[agent_id].hand += hypothetical.board.draft
        hypothetical.board.draft = ["##"]
        print(hypothetical.agents[agent_id].hand)
        #hypothetical.agents[agent_id].hand = hypothetical.agents[agent_id].hand[0:2]
        best_card = None
        best_cord = None
        best_state = None
