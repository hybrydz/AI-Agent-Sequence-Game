...
class myAgent(Agent):
    ...
    def SelectAction(self, actions, game_state):
        ...
        # jacks rule - save for high impact plays
        pre_action_win = get_win_percentage(state.board.chips, your_char, their_char, special_char)
        jacks = ['jd','jc','jh','js']
        jack_threshold = 0.005
        ...
        for action in nodraft_actions:
            ...
            succ_score = get_win_percentage(succ_state.board.chips, your_char, their_char, special_char)
            if succ_score > highest_score:
                if action['play_card'] in jacks:
                    if succ_score > (pre_action_win + jack_threshold):
